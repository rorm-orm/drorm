...
//! It the proceeds to implement then [`ModelField`] trait on this empty struct.
//! Therefore, [`ModelField`] encapsulates all information the macro can gather.
//! This includes:
...
pub trait ModelField: Field<Struct: Model> {
    type Model: Model;
}

impl<T: Field<Struct: Model>> ModelField for T {
    type Model = T::Struct;
}

/// Pushes a [`ModelField`]'s columns as [`imr`] onto a vector.
///
/// This function is called by the `#[derive(Model)]` macro to gather a list of all vectors.
pub fn push_imr<F: ModelField>(imr: &mut Vec<imr::Field>) {
        ...
        .zip(db_types.into_iter())
    {
        ...
        imr.push(imr::Field {
            ...
            source_defined_at: Some(imr::Source {
...
/// Check a [`ModelField`] for correctness by evaluating its [`FieldType`]'s `Check`
///
/// This function is called and its error reported by the `#[derive(Model)]` macro.
pub const fn check<F: ModelField>() -> Result<(), ConstString<1024>> {
...
pub trait SingleColumnField: ModelField {
...
impl<F: ModelField, P> FieldProxy<F, P> {
...
pub trait ContainerField<T: FieldType, P: Path>: ModelField<Type = T> {
    /// Struct of contained fields
    type Target: ConstNew;
}

impl<T: FieldType, F: ModelField<Type = T>, P: Path> std::ops::Deref for FieldProxy<F, P>
where
...
where
    T: FieldType,
    F: ModelField<Type = T> + PathField<T>,
    P: Path<Current = <F::ParentField as ModelField>::Model>,
{
    type Target = <<F::ChildField as ModelField>::Model as Model>::Fields<P::Step<F>>;
}

/// Helper structs implementing [`Contains`] to expose
/// - [`ModelField::NAME`]
/// - [`ModelField::EXPLICIT_ANNOTATIONS`]
/// - [`ModelField::EFFECTIVE_ANNOTATIONS`]
mod contains {
    use std::marker::PhantomData;
    ...
    pub struct ExplicitAnnotations<F: ModelField>(PhantomData<F>);
    impl<F: ModelField> Contains<Annotations> for ExplicitAnnotations<F> {
        const ITEM: Annotations = F::ANNOTATIONS;
    }

    pub struct EffectiveAnnotations<F: ModelField>(PhantomData<F>);
    impl<F: ModelField> Contains<FieldColumns<F::Type, Annotations>> for EffectiveAnnotations<F> {
        const ITEM: FieldColumns<F::Type, Annotations> = F::EFFECTIVE_ANNOTATIONS;
    }

    pub struct Name<F: ModelField>(PhantomData<F>);
    impl<F: ModelField> Contains<&'static str> for Name<F> {
